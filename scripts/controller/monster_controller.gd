...
func adjust_monster_hitpoint(monster: Monster, ammount: int):
	monster.hp = clamp(monster.hp + ammount, 0, monster.max_hp)
...
func use_monster_move(monster: Monster, move: Move):
	if move.usages <= 0 or monster.hp == 0:
		#Events.request_log.emit("Moves ended")
		return
	...
	if !hit:
		...
		crit = rng.randf() < Calculations.get_crit_chance(monster)
		if crit:
...
func create_monster(species: SpeciesResources, nickname: String = "") -> Monster:
	...
	for move_resource in species.starter_move:
		if move_resource == null:
			continue
		var move = Move.new()
		move.resource = move_resource
		move.usages = move_resource.max_usage
		moves.append(move)
	
	monster.moves = moves
	
	monster.fallback_move = Move.new()
	monster.fallback_move.resource = load("res://scripts/content/moves/struggle.tres") as MoveResource
...
func instantiate_condition_on_monster(monster: Monster, condition_resource: ConditionResource):
	if monster.conditions\
	 .filter(func(condition): return (condition.resource == condition_resource and condition_resource.status_condition))\
	  .size() >= condition_resource.max_status_stacks:
		return
	elif  monster.conditions\
	 .filter(func(condition): return condition.resource == condition_resource)\
	  .size() >= condition_resource.max_stats_stacks:
		return
