...
func setup_model():
	...
	var species_salamander = preload("res://scripts/content/species/salamender.tres")
	var species_turtle = preload("res://scripts/content/species/turtle.tres")
	var species_dino = preload("res://scripts/content/species/dino.tres")
...
func handle_run():
	if current_phase != PHASE.AWAIT_INPUT:
		return
	Events.request_log.emit("You run away, coward")
...
func choose_ai_move() -> Move:
	...
	if legal_move_indices.size() <=0:
		Events.request_log.emit("No move possible. Using default")
...
func resolve_round():
	...
	# Sicurezza: a fine round nessun mostro deve conservare la mossa scelta.
	# Altrimenti _process() rientra subito in RESOLVE_ROUND al frame successivo.
	game_state.player_monster.chosen_move = null
	game_state.opponent_monster.chosen_move = null
	
	if game_state.player_monster.hp == 0:
		var next_index = TrainerController.get_next_usable_monster_index(game_state.player)
		if next_index == -1:
			current_phase = PHASE.GAME_OVER
	...
	if game_state.opponent_monster.hp == 0:
		var next_index = TrainerController.get_next_usable_monster_index(game_state.opponent)
		if next_index == -1:
			current_phase = PHASE.GAME_OVER
