...
func _process(_delta: float) -> void:
	if active and current_effect_group == null:
		if effect_group_queue.size() == 0:
			...
			# Se il gruppo è vuoto, non ci sono effetti da eseguire.
			# Quindi lo eliminiamo subito e lasciamo che _process
			# passi al gruppo successivo o chiuda il blocco AVFX.
